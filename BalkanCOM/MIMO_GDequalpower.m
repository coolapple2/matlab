...
for D=1:10
    ...
    for monte=1:iter   
        ...
        while 1==1      
            ...
            for k=1:K
                Yk(k)=(Ythr(k)/(Pbsr*lambda(D+1)))*(sum(lambda(1:D).*Pdr.*(abs(g(:,k)).^2))+No);
            end
...
GD_EqualPower_D2Dsumrate_withoutprecoder=mean(D2Dratearray_varyingD); %average D2Dsum rate with BF precoder
CUEsumrate_withoutprecoder=mean(CUEratearray_varyingD); %average sum rate with BF precoder
