...
for D=10:10
    ...
    for monte=1:iter
        ...
        if min(SNRCUENoD2D)<Yth
            Yth=min(SNRCUENoD2D);
        end
        %pause
        SNRCUE=zeros(K,1)';
        %% Distributed
        while (abs(1-Pmin/Pmax)>0.001 || min(SNRCUE)<Yth)
            price=0.5*(Pmin+Pmax);
            ...
            for d=1:D
                ...
                lambda(d)=Pdr*((abs(rho(d,d)))^2/(price*Pdr*sum(abs(g(d,:)).^2))-1)*InterfD2D(d);
                %lambda(d)=Pdr*((abs(rho(d,d)))^2-InterfD2D(d))/(price*Pdr.^2*sum(abs(g(d,:)).^2)*(abs(rho(d,d)))^2);
                if lambda(d)>1
                    lambda(d)=1;
                elseif lambda(d)<0
                    lambda(d)=0;
...
% legend('Prec=BF PA=Distributed')

%     figure
%     cdfplot(10*log10(CUESNRarray))
CUESNRarrayz=[CUESNRarray(:,1);CUESNRarray(:,2);CUESNRarray(:,3);CUESNRarray(:,4);CUESNRarray(:,5)]
figure
cdfplot(10*log10(CUESNRarrayz))
save(['BF_Distributed_iter_'  '_alg_' date '.mat'])
toc
