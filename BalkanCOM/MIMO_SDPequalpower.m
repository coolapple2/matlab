...
for monte=1:iter 
    ...
    for D=1:10
        ...
        while 1==1
            Yk=zeros(1,K); 
            for k=1:K
                Yk(k)=(Ythr(1)/(Pbsr*lambda(D+1)))*(sum(lambda(1:D).*Pdr.*(abs(g(:,k)).^2))+No); %Equation 8c
            end
        ...
        if opt2 ~= Inf
            %% Algorithm 1 SDP algorithm for precoding 
            [Vx,Dx]=eig(X);
            Vxx=eig(X)
...
SDP_EqualPower_D2Dsumrate_withoutprecoder=mean(D2Dratearray_varyingD); %average D2Dsum rate with BF precoder
SDP_CUEsumrate_withoutprecoder=mean(CUEratearray_varyingD); %average sum rate with BF precoder

%Plot figures
figure('Name','Sum rate of D2D pairs','NumberTitle','off')
plot(SDP_EqualPower_D2Dsumrate_withoutprecoder,'-o','color','g')
grid;
xlabel('Number of D2D pairs (D)');
ylabel('Average Sum rate of D2D pairs (bits/sec)');
title('Sum rate of D2D pairs')
legend('Prec=SDP PA=Uniform')
