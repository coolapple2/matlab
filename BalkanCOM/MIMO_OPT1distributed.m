        ...
        for iteralgrz=1:iter_algorithm
            ...
            while (abs(1-Pmin/Pmax)>0.001 || min(SNRCUE)<Ythr)
                ...
                for d=1:D
                    ...
                    minimize norm(trace(F{1}*X)) + norm(trace(F{2}*X)) + norm(trace(F{3}*X))+ norm(trace(F{4}*X))+ norm(trace(F{5}*X)) + norm(trace(F{6}*X))...
                        +norm(trace(F{7}*X))+ norm(trace(F{8}*X))+norm(trace(F{9}*X))+norm(trace(F{10}*X))
                    trace(X) == 1;
                    X >= 0;
                    for k=1:K
                        (h(:,k)*h(:,k)')*X-Yk(k)>=0
