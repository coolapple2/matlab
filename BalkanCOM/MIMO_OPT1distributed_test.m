...
No   = 10^(-10);
Yth  = 0.96;   % Threshold SINR for CUEs dB
Ythr = 0 %10^(Yth/10);
price=10^-4;

iter=5       % Number of montecarlo iterations
        ...
        for iteralgrz=1:iter_algorithm
            ...
            while (abs(1-Pmin/Pmax)>0.001 || min(SNRCUE)<Ythr)
                ...
                for d=1:D
                    ...
                    for d=1:D
                        cost= cost+trace((A(d)-B(d))/(A(d)*B(d))*f(:,d)*f(:,d)'*X)
                    end
                    minimize cost
                    %sum(F(:real( trace(F{1}*X) + trace(F{2}*X) + trace(F{3}*X)+ trace(F{4}*X)+ trace(F{5}*X) + trace(F{6}*X)...
                     %   +trace(F{7}*X)+ trace(F{8}*X)+trace(F{9}*X)+ trace(F{10}*X))
                    trace(X) == 1;
                    X >= 0;
                    for k=1:K
                        (h(:,k)*h(:,k)')*X-Yk(k)>=0
...
legend('Prec=GD PA=Distributed')

figure
cdfplot(10*log10(CUESNRarray))

toc
save('GDdistributed2.mat')
